...
double my_random_double()
{
    ...
    // generate random bits until we see the first set bit
    while (1) {
      x = random(); //*******random is a 32 bit number, normally. 
      x <<= 32; //*******want to shift over x 32 bits over to make it 64 
	  y = random(); //want to add an extra random number to original after you've pushed over the bits to fill in the 0s
	  x+=y; //you want to fill in the 0s because you don't want the extra numbers/ space because it's now a double (which has more space). the larger the number you're dividing with, the more variation you can get. 
	  if (x == 0) {
        exp -= 63; // (32 *2) -1 
      } else {
        break;
    ...
    // use the remaining bit as the mantissa
    mant = x >> 8;
    b.i = (exp << 23) | mant; //<< shifts over bits, 
